...
def loadDataSet(train_data_path,train_label_path,test_data_path,test_label_path, 
	train_data_num = 60000, test_data_num = 10000, fig_w = 45):
...
def pca_algorithm(dataSet , d = 30):
	print 'Doing PCA......'
	print 'Origin data size:', dataSet.shape
	pca = PCA(n_components=d)
	pca.fit(dataSet)
	new_data = pca.transform(dataSet)
	print 'After PCA data size:', new_data.shape
	return new_data
...
def ica_algorithm(dataSet , d = 0.95):
	...
	print 'After ICA data size:', new_data.shape
	return new_data
